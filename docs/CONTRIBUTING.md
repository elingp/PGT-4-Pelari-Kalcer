...
Welcome! This guide explains how we collaborate on the RunCam project. The goal is to keep the workflow lightweight, predictable, and friendly for contributors at any experience level.
...
- **Language:** Write code, commits, and documentation in English when possible. Indonesian is fine in discussion threads if it helps the team move faster, but keep code and docs readable for everyone.
...
3. Start the supporting services with `bun run db:up`, then reset the database with `bun run db:reset`.
...
3. **Develop & document**
   ...
   - Update tests, docs, and sample data when behavior changes.
   - Run key quality checks locally:

     ```bash
     bun run check      # lint + format + static checks
     bun run db:reset   # optional safety net for schema/seed changes
     bun run build      # optional, recommended when you touch build/config tooling
     ```
...
| `refactor` | Internal change without behavior shift | `refactor(core): split env parser`   |
